# Message dispatch and transport framing: historical record

This file is the chronological record behind the consolidated topic notes in this
directory. It retains the original framing note and every addendum **verbatim**,
including superseded claims and the corrections that followed. Read the topic notes
for the current state; use this file for provenance and the reasoning trail.
...
indirect call. The Ghidra symbol name `PTR_LAB_142109a38` is misleading  Ethe slots are
...
The original note stopped at framing. The layer above it  Ehow a frame payload becomes a
typed message and reaches a handler  Ehas now been recovered. This addendum supersedes the
...
mixed in  Ethat is still the key schedule at `140feea50`.
...
only reached in modes 1 and 2.** In mode 3 the loop never touches it  Eit goes straight to
...
## Msg::DispatchStream  Ethe message layer

`Msg_DispatchStream` @ `140fe9120` (`MsgConn.cpp`), per message:

1. `MsgUtil_ReadBytes(ctx+0x80, 2, 4, &msgId, 0)`  Eread a **u32 message id**. This is the
   first field of the payload; ids are read as 4 bytes, little-endian into a qword-sized dest.
2. `Msg_RegistryRecordRecv(registry, msgId)`  Eflood accounting, window 1000 ticks
   (`FUN_1409cdda0` clock). Exceeding it raises `ERR_FLOODING` (line `0xb32`).
3. `Msg_RegistryLookupById(registry, msgId)`  Eresolve the record. Miss raises
...
**`msgId` indexes the table directly  Eno hashing, no binary search.** Message ids are
...
- `0`  E`handler(param_3, decoded)`
- `1`  E`handler(*param_3, decoded)` (dereferences the connection context first)
...
Type-name values are build-local. Re-derive them for any other build  Edo not carry the
numbers across.
...
writer does **not**  Eit trusts the caller to have pre-sized via `ComputeMaxSize`. That
...
2. Dump the `defArray` chain per slot across `0..count-1` and emit it as a machine-readable
   schema corpus  Ethis is now mechanical, not investigative.
...
now located. This does **not** close that step  Ethe *runtime* record array is still not read  Ebut the static tables that populate it are, and they yield concrete message ids.
...
| `maxSize <= MSG_MAX_BUFFER_SIZE` | `0x81` | **0x2000  Ethe binding bound** |
...
| Offset | Type | Field |
| --- | --- | --- |
| `0x00` | `void*` | `defArray`  Ethe MsgPackFieldDef chain, or points into `.text` for descriptors embedded in code |
| `0x08` | `void*` | `handlerFn`  Edispatch target, called `(ctx, decoded)` |
...
**Total 59 registration sites.** `MsgChannel_RegisterTablePair` was previously unknown  Eit is
...
is not a message identity  Eone handler can serve several ids.
...
2. Check `maxSize <= 0x2000` across the extracted corpus, and treat a violation as a decode bug
   rather than a game bug  Eit would mean a descriptor offset is wrong.
...
| `141246040` | `.text` | `FUN_141246040` |  E|  E|
...
The hand-verified 8 entries from `ch14-recv.json`  Ecommitted *before* the script existed  Eare
...
| --- | --- | --- |
| 0 E09 | `MP_MSGID` schema (220/220 in col A overall) | `MP_MSGID` schema |
| 110 E19 | `MP_MSGID` schema | pointer into `.text`, inside a function |
...
- **176 distinct ids across 220 elements**  Eso ids repeat; the element index is not the id
...
- `maxSize <= 0x2000` is still unchecked against the corpus  Ethe extractor does not yet compute it.
- **Still no wire fixture.**

## Next steps

1. Re-run the corrected extractor over the send table `142167710` (624 elements).
2. Characterize the `.text` pointers in column B, indices 110 E19: resolve them to functions.
...
21 / 36 / 2  E**independently reproducing** the instruction-search count from Addendum 3 by a
...
emitted `?` for every table while still decoding the `R8D` counts correctly  Ea signature that
...
**Site `14021cf01` is the worst**  E16 rows, all suspect. Its reported "pointers" include
...
- `maxSize <= 0x2000` is still unchecked  Ethe extractor does not compute it.
...
| both |  E| `defSize <= MAX_WORD` | `:0x7d` |
| both |  E| `maxSize <= MAX_WORD` | `:0x80` |
| both |  E| `maxSize <= MSG_MAX_BUFFER_SIZE` | `:0x81` |
...
the "every other id" symptom of `1c, 1f, 21, 23, 25`  EI attributed it to a `String.format`
...
| --- | --- | --- |
| 8 (flat) | 220 x 8 = `0x6E0` | `142167030 + 0x6E0` = **`142167710`**  Eexactly table B's start |
...
at byte offset `110 x 16 = 1760` = `0x6E0`  E**exactly the end of table A**. So "index 110" was
...
| --- | --- | --- |
| `RegisterRecvOnly` (`140fed7f0`) | pairs (16) |  E|
...
| `CODE` | 1305 | pointer into `.text`  Ethe recv table's dispatch handler |
...
## 2. `0x264` is found  Eand it is a recv message
...
the recv registry**  Ethe "identity-mapping" open question is answered for this message: the
...
entirely recv-side  Econsistent with its name and with the sweep's `col1` being `.text` handlers.
...
No violation. This was the expected result  Ethe client's own validator asserts the bound at
startup (`MsgChannel.cpp:0x81`)  Eso the value of the check is that a **wrong descriptor offset
...
`maxSize` at `+0x1C` and `nextDef` at `+0x20`  Eevery composite field one qword low. A consumer
...
the preceding login/platform exchange  Ei.e. it is issued to the client, not invented by the
...
offline, a capture must also supply the session state  Eeither
...
then, at `+0x28`, `4`, then `4`, then `0xc`  Ethe same `0x28`-stride, `fieldType`-first format as
...
# Addendum 13: the connection seed is client-generated

**Status:** both connection openers and the client-side seed generator are located; Addendum 11's
"provisioned, not client entropy" reading is **not supported and is retracted**; **still no wire
fixture**

This addendum continues the Addendum 12 writer search. It did not isolate the instruction that
writes the game request's `obj+0x7c`, but it found the seed's origin class: the seed is a
client-generated 20-byte value, not server-issued.

## The two connection openers both take a seed at `+0x34`

Both `MsgConn` openers read their key seed from `+0x34` of a request/parameter structure:

| Opener | Source unit | Protocol | Callback | Seed |
| --- | --- | --- | --- | --- |
| `FUN_140240cf0` (`Gc::GcAuthCmdNotifyProc`) | `GcAuthCmd.cpp` | `3` | `FUN_140240a50` | `param_4 + 0x34` (= request) |
| `FUN_14023ed80` | `GcConn.cpp` | `0` | `Gc::GameSrvEncryptCallback` | `request + 0x34` |

`FUN_140240cf0` is called from the same place as `FUN_14023ed80`: the game-connect builder
`FUN_14023fca0` loads a function pointer at `puVar6+0x12` and calls it with
`(1, handle, 0x48, request, ...)`, choosing `FUN_14023ed80` for request mode `0` and
`FUN_140240cf0` for mode `3` (its internal switch and `FUN_14023f9b0` case 1 agree). So both
openers receive the **same request** and read the **same seed at `request+0x34`**; the mode only
selects protocol `0`/`3` and the callback.

## The seed is generated by client-side entropy

`FUN_140fdf2d0`, the 20-byte time/entropy generator, has exactly **two** callers in the image:

| Caller | Use |
| --- | --- |
| `FUN_140fe9b00` | `MsgConn` constructor, when the supplied seed length is `0` |
| `FUN_140243380` | auth connect callback, `param_2 == 1` |

`FUN_140243380` is the asynchronous auth-connection callback wired by `FUN_1402437d0` via
`FUN_140fe3b90`. On the connect state (`param_2 == 1`) it builds a payload at `&local_88`, calls
`FUN_140fdf2d0(0x14, local_54)`, and fires event `2`. Its payload layout matches the connection
request used by the openers:

| Payload offset | Request offset | Field |
| --- | --- | --- |
| `+0x00` | `+0x00` | pointer |
| `+0x08` | `+0x08` | pointer |
| `+0x10`..`+0x2f` | `+0x10`..`+0x2f` | 28-byte block (from `obj+0x6c`) |
| `+0x30` | `+0x30` | `u32` |
| `+0x34` | `+0x34` | **20-byte seed**, filled by `FUN_140fdf2d0` |

So the seed is **fresh client randomness** placed at `request+0x34` and used by whichever
connection the builder opens. The earlier "seed is not fresh client entropy" claim was drawn from
the absence of a write at the additive offset `+0x7c` and is wrong: the generation happens at
`+0x34` of the request, and `+0x7c` was only how that field appeared relative to the game event's
`obj+0x38` payload base.

## What this does not establish

- The exact instruction that writes the *game* request's `obj+0x7c`. It is consistent with the
  auth callback's random, but whether the game connection reuses the auth random or has its own
  generation was not traced.
- Whether event `2`'s auth-context handler (distinct from the MainCli `FUN_140237a30` that was
  read) is the consumer that forwards the random into the request.
- **Still no wire fixture.**

## Correction to earlier addenda

- Addendum 11's conclusion "the seed is provisioned state, not fresh client entropy ... issued to
  the client" is **retracted**. The located evidence supports the opposite: the client generates
  the 20-byte seed with `FUN_140fdf2d0` during connection setup.
- Addendum 11's capture requirement is unchanged in *effect* (a capture still needs
  `conn+0x118` or the seed), but its rationale changes: the seed is not required because it is
  secret, but because it is per-session random and not recoverable from the wire alone.

## Next steps

1. Trace the auth-context event-`2` handler to confirm it stores the `+0x34` random into the
   `MsgConn` request consumed by `FUN_140240cf0`.
2. Determine whether the game opener `FUN_14023ed80` reuses that random or generates its own;
...
table  Ethe runtime registry record points at the same shipped `defArray`.
...
**9 messages that consume exactly 1807 bytes**  Eno desync, no leftover:
...
data. (The plaintext is not a message start  Ethe reader buffers across packets, so no `msgid` is
...
full 256-byte S-box coinciding by chance is not credible, so the key schedule  Ethe 20-byte
XOR buffer, the five-word mixing, and the RC4 KSA  Eis reproduced as well.
...
`14020a031` gave a long message that ran past the 265-byte capture  Econsistent with the stream
...
The live record for `0x48` is `1425AE2E0`  Ethe **first-row** defArray, i.e. exactly what the naive
...
id. The run is therefore **not** confirmed as a real boundary  Eit may be coincidental  Eand the
...
- the payload is decoded  Ecopied raw when the method word is `0`, otherwise through
  `FUN_141574a00`  Einto an intermediate buffer;
...
## The seed (`FUN_140fdf2d0`)  Eclient entropy, not wire-derivable
...
- verification against a captured `(seed, outA)` pair  Ethe reconstruction is static and the
...
Base `+0x50`, count `+0x5c`, 16-byte records, `defArray` at `+0x08`  Ethe **table-A (send)** record
...
- **Inbound** `FUN_140fe8ef0` wraps payloads in `[u16 compLen][u16 decodedLen]` + LZ4 (Addendum 20).
  **Outbound** at this level encrypts the raw message stream and hands it to the transport  Eno
...
is never true in a live session. Record it as a transport-layer weakness  Eit does not block
...
below this boundary. The constructor corroborates this  Eit builds the handshake `outB` message as
...
client entropy (`R9 = conn+0x118`), so the conclusion is unchanged  Ea capture still needs the seed
or the key.
...
`[u16 msgid][fields]` stream  Eno `[compLen][decodedLen]` and no LZ4 on the outbound side  Eand hands
...
1. Extract the send schema corpus (table A) as a build artifact, analogous to `chains-recv.json`,
   and extend the runtime decoder to take a direction-specific corpus. (Done  EAddendum 27.)
...
varint]` in the recv corpus  Ethe send chain is the one that decodes the captured outbound packet
