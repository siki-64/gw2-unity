#!/usr/bin/env python3
"""Reference implementation of the handshake key derivation.

Recovered statically from Gw2-64.exe (see the schema re-derivation):
...
# Captured live (the schema re-derivation): the game-connection seed and the
...
def selftest():
    ...
    # Captured live vector: seed -> outA[:20] must reproduce conn+0x118.
...
def emit_fixture(path):
    ...
    body = {
        ...
        "provenance": "Generated by tools/re/Gw2HandshakeKdf.py from the static "
                      "reconstruction in the schema re-derivation.",
        "capturedVector": {
            "seed": CAPTURED_SEED,
            "outA20": CAPTURED_OUTA20,
            "provenance": "Live game connection, (the schema re-derivation): "
