#!/usr/bin/env python3
"""Emit a message-schema corpus (recv or send) as JSON.

Inbound and outbound have different chains per id (the schema re-derivation), so
each direction gets its own corpus:

  recv  every id in the live per-connection recv registry map
        (protocol/schema/207032/live_ids.csv);
...
SCHEMA_DIR = os.path.join(ROOT, "protocol", "schema", "207032")
...
def main():
    ...
    if args.direction == "recv":
        ...
        source = "protocol/schema/207032/live_ids.csv (live game-connection recv registry)"
    else:
        id_map = send_id_map(args.registrars, args.sweep)
        source = ("protocol/schema/207032/sweep2.csv table A (flat send tables) split by "
    ...
    body = {
        ...
        "provenance": "Generated by tools/re/ExtractSchemaCorpus.py (the schema re-derivation offsets).",
        "messages": messages,
    }
